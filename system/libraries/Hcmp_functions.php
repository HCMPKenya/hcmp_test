...
public function get_county_email($county_id){
	...
	return 'karsanrichard@gmail.com';
	}else{
...
if(count($excel_data)>0):
 ...
 public function clone_excel_order_template($order_id,$report_type,$file_name=null){
	...
	$sysnthesis = array();
	foreach ($facility_stock_data_item as $fsd) {
		if (isset($fsd['quantity_ordered_pack']) && $fsd['quantity_ordered_pack'] > 0) {
			$sysnthesis[] = array(
				'commodity_name' => $fsd['commodity_name'], 
				'commodity_code' => $fsd['commodity_code'], 
				'quantity_ordered_pack' => $fsd['quantity_ordered_pack']
				);
		}
	}
...
for ($row = 17; $row <= $highestRow; $row++){ 
   ...
   if(isset($rowData[0][2]) && $rowData[0][2]!=''){
	...
	$excel2->getActiveSheet()->setCellValue("H$row", $facility_stock_data_item_['quantity_ordered_pack']);	
   	// echo "<pre>";print_r($facility_stock_data_item_['quantity_ordered_pack']);echo "</pre>";
   	}	
   	} 	
   }
}
// exit;
 ...
 public function clone_meds_excel_order_template($order_id,$report_type,$file_name=null){
    $inputFileName = 'print_docs/excel/excel_template/MEDS Customer Order Form.xls';
    $facility_details = facility_orders::get_facility_order_details($order_id);
    // echo "<pre>";print_r($facility_details);echo "</pre>";exit;
	if(count($facility_details)==1):
	$facility_stock_data_item = facility_order_details::get_order_details($order_id);
    ...
    // $highestColumn = $sheet->getHighestColumn();
    $highestColumn = 'L';//due to its weird format
...
for ($row = 16; $row <= $highestRow; $row++){ 
   ...
   if(isset($rowData[0][3]) && $rowData[0][3]!=''){
   	// echo "<pre>";print_r($rowData[0][3]);echo "</pre>";
   	foreach($facility_stock_data_item as $facility_stock_data_item_){
   	if($rowData[0][1] == $facility_stock_data_item_['commodity_code']){
   	// echo "<pre>";print_r($rowData[0][3]);echo "</pre>";
   	// $key = array_search($rowData[0][3], $facility_stock_data_item_);
	$excel2->getActiveSheet()->setCellValue("I$row", $facility_stock_data_item_['quantity_ordered_pack']);	
   	}
   	} 	
   }
}
// exit;
		// $report_type='download_file';
   $objWriter = PHPExcel_IOFactory::createWriter($excel2, 'Excel5');
   if($report_type=='download_file'){
   	// We'll be outputting an excel file
		header("Last-Modified: " . gmdate("D, d M Y H:i:s") . " GMT");
